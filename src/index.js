...
const main = async () => {
  ...
  // Register/Log in
  const selectedLink = (await term.singleColumnMenu(['Login', 'Add Wallet'])).selectedText

  // Username & password.
  let username = await term.input('Username')
  let password = await term.input('Password', { echoChar: true })

  // Open Wallet and connect to Lorena
  let lorena = new Lorena(new Wallet(username), { debug: true, silent: true })
  ...
  if (selectedLink === 'Login') {
    ...
    // Add Wallet
    if (await term.yesOrNo('Wallet does not exist. Create a new wallet?')) {
